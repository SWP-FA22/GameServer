...
public class PostModel extends ModelBase<Post> {
   ...
   public boolean createPost(int createdby,  String title, String description) throws Exception{
    ...
    public List<Post> getPostByID(Integer id) throws Exception {
        List<Post> listbyid = new ArrayList<>();
        try ( ResultSet rs = ModelBase.connection().executeQuery(" SELECT * FROM [Post] WHERE [ID] = ?", id)) {
            while (rs.next()) {
                Post post = new Post();
                post.loadProps(rs);
                listbyid.add(post);
    ...
    public void updateStatusPost(Post p) throws Exception{
        ModelBase.connection().executeUpdate("UPDATE [Post] SET [isApproved] = ? WHERE [ID] = ?", p.getIsApproved(), p.getId());
