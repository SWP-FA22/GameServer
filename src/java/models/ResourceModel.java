...
public class ResourceModel extends ModelBase<Resource> {
    ...
    public Map<Resource, Integer> getAllResourceOfPlayers(Integer pid) throws Exception {
        Map<Resource, Integer> result = new HashMap<>();

        try ( ResultSet rs = connection().executeQuery(
                "SELECT [Resource].*, ISNULL((SELECT [Amount] FROM [PlayerResource] WHERE [PlayerID] = ? AND [Resource].[ID] = [ResourceID]), 0) AS [Amount] FROM [Resource]", pid)) {
            while (rs.next()) {
                Resource resource = new Resource();
                resource.loadProps(rs);

                result.put(resource, rs.getInt("Amount"));
            }
        }
        return result;
    }
